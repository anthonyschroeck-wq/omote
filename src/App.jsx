...
export function PerformanceShell({ session }) {
  useEffect(() => {
  ...
  // Validate session has renderable content
  if (!session?.cue?.jsxCode && !session?.cue?.shellHtml) {
    return (
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Source Sans 3',sans-serif", color:"#4A4A44" }}>
        <div style={{ textAlign:"center", maxWidth:400 }}>
          <div style={{ fontSize:48, marginBottom:16, opacity:0.3 }}>⚠</div>
          <h2 style={{ fontFamily:"'Instrument Serif',serif", fontSize:24, marginBottom:8 }}>Session Not Found</h2>
          <p style={{ fontSize:14, color:"#8B8880", lineHeight:1.6 }}>This performance link has expired or the session data wasn't found. Go back to Omote and click Perform again.</p>
          <button onClick={()=>window.close()} style={{ marginTop:20, padding:"10px 24px", background:"#1A1A1A", color:"#F5F0E8", border:"none", fontFamily:"'IBM Plex Mono',monospace", fontSize:11, cursor:"pointer" }}>Close Tab</button>
        </div>
      </div>
    );
  }
...
export default function Omote() {
  ...
  const applyPrefs = (profile) => {
    const prefs = profile?.flags?.prefs;
    if (!prefs) return;
    if (prefs.themeMode) setThemeMode(prefs.themeMode);
    if (prefs.pointerConfig) setPointerConfig(prev => ({ ...prev, ...prefs.pointerConfig }));
  };

  // ─── Save prefs to Supabase (debounced) ───
  const savePrefsTimer = useRef(null);
  const savePrefs = useCallback((newTheme, newPointer) => {
    if (!user?.id) return;
    clearTimeout(savePrefsTimer.current);
    savePrefsTimer.current = setTimeout(() => {
      const currentFlags = user.flags || {};
      const newFlags = { ...currentFlags, prefs: { themeMode: newTheme, pointerConfig: newPointer } };
      db.updateProfile(user.id, { flags: newFlags }).catch(() => {});
    }, 800);
  }, [user?.id, user?.flags]);

  // ─── Wrapped setters that persist ───
  const setThemePersist = (m) => { setThemeMode(m); savePrefs(m, pointerConfig); };
  const setPointerPersist = (c) => { setPointerConfig(c); savePrefs(themeMode, c); };
  ...
  const launchPerformance = (stage, cue, company) => {
    ...
    // Use localStorage (shared across tabs) instead of sessionStorage (per-tab, unreliable with window.open)
    localStorage.setItem("omote-perf-" + id, JSON.stringify(session));
    // Clean up stale sessions (older than 1 hour)
    Object.keys(localStorage).filter(k => k.startsWith("omote-perf-")).forEach(k => {
      try { const d = JSON.parse(localStorage.getItem(k)); if (d.ts && Date.now() - d.ts > 3600000) localStorage.removeItem(k); } catch { localStorage.removeItem(k); }
    });
